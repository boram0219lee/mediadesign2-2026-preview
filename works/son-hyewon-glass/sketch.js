...
function updateCreature() {
  if (jumping) {
    ...
    vibrateAmt = 0; sizeMult = lerp(sizeMult, 1.0, 0.05);
  } else {
    let d = dist(worm[0].x, worm[0].y, mouseX, mouseY);
    if (d < fleeRadius) {
      let fleeVec = createVector(worm[0].x - mouseX, worm[0].y - mouseY).normalize();
      targetX = worm[0].x + fleeVec.x * 90; targetY = worm[0].y + fleeVec.y * 90;
      worm[0].x = lerp(worm[0].x, targetX, 0.07); worm[0].y = lerp(worm[0].y, targetY, 0.07);
    } else {
      targetX = lerp(targetX, mouseX, 0.015); targetY = lerp(targetY, mouseY, 0.015);
...
function drawMouseFleeBoundary() { push(); noFill(); stroke(255, 65); strokeWeight(1.5); drawingContext.setLineDash([5, 8]); circle(mouseX, mouseY, fleeRadius * 2); drawingContext.setLineDash([]); fill(255, 30); noStroke(); circle(mouseX, mouseY, 4); pop(); }
