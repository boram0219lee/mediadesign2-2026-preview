...
const INTERACT_CONFIG = {
  ...
  targetStopRadius: 150,     // 사람에게 다가갈 때 브레이크를 밟기 시작하는 '안전거리 반경' (떡짐, 떨림 방지)
  
  targetLerp: 0.1,           // 생명체가 관람객의 위치를 쫓아올 때의 부드러움 (1에 가까울수록 즉각적)
  
  fleeThreshold: 11.5,       // [상태 2 도망침] 관람객의 움직임 수치가 이 숫자 '이상'으로 오르면 기겁하고 도망갑니다.
...
const STABILITY_CONFIG = {
  ...
  motionSensitivityUp: 0.1,    // 움직임을 감지할 때 수치가 팍! 차오르는 속도
  motionSensitivityDown: 0.03, // 움직임이 멈췄을 때 수치가 스르륵 내려가는 속도

  // 3. 상태 전환 대기 시간 (버벅거림 방지)
  // 도망(상태 2)치다가 관람객이 가만히 멈췄을 때, "아, 진짜로 멈췄구나"라고 확신하기까지 기다리는 시간입니다.
  // - 도망치다가 다시 다가오는 데까지 너무 오래 걸리면 -> 수치를 낮추세요 (예: 15 ~ 20)
  // - 상태가 1, 0, 2를 오가며 덜덜 버벅거리면 -> 수치를 올리세요 (예: 60)
  calmDownFrames: 30,        
...
const BOID_CONFIG = {
  ...
  // ⭐️ [수정됨] 이제 모든 상태에서 아래의 범위 안으로 겹치면 반짝입니다.
  sparkleDistanceState1: 130, // 서로 부딪혔다고 인식할 범위
...
const STATE_CONFIG = {
  0: { 
    ...
    speedMultiplier: 0.5,     // 기준 속도의 20%로 유유자적하게 돎
    alphaTarget: 220,         // 투명도 (약간 반투명)
    lerpScale: 0.01,          // 크기 변화 속도
    lerpSpeed: 0.05,          // 속도 변화 속도
    lerpAlpha: 0.02,          // 투명도 변화 속도
    separationDist: 50,       // [최소 유지 거리] 평소 겹치지 않게 밀어내는 거리
    weights: { sep: 2.0, ali: 1.0, coh: 0.1, seek: 0.0 } 
  },
  1: { 
    scaleTarget: 4.5,         // 호기심에 5배로 부풀어 오름
    speedMultiplier: 0.15,    // 사람을 관찰하느라 아주 느려짐
    alphaTarget: 255,         // 선명하게 드러남
    lerpScale: STABILITY_CONFIG.growSpeedToCurious, // ⭐️ 조절 패널에서 설정한 속도로 적용됩니다!
    lerpSpeed: 0.05,          
    lerpAlpha: 0.008,         
    separationDist: 90,       // [최소 유지 거리] 몸집이 커졌으므로 떡지지 않게 거리 벌림
    weights: { sep: 3.0, ali: 0.5, coh: 0.1, seek: 2.5 } // 사람을 향해 다가오며 둥글게 에워쌈
  },
  2: { 
    ...
    weights: { sep: 4.0, ali: 0.5, coh: 0.1, seek: -2.0 } // 거칠게 튕겨내며 사람 반대로 도망침
  }
};
...
const RECOVERY_CONFIG = {
  lerpScale: 0.0005,  // 놀란 가슴을 진정하며 원래 크기로 부풀어 오르는 속도 (아주 천천히)
  lerpSpeed: 0.0002,  // 미친듯이 도망치던 가속도가 평온해지는 속도 (낮을수록 관성이 오래감)
...
function analyzeAudience() {
  let validPoses = poses.filter(p => {
    ...
    let areaA = a.box ? (a.box.width * a.box.height) : 0;
    let areaB = b.box ? (b.box.width * b.box.height) : 0;
    return areaB - areaA; 
  });
...
class Boid {
  ...
  // ⭐️ [변경됨] 이제 상태(0, 1, 2)에 상관없이 무조건 거리만 가까우면 반짝거립니다!
  ...
  flock(boids, state, target) {
    ...
    sep.mult(settings.sep); ali.mult(settings.ali); coh.mult(settings.coh);
    seekForce = this.seek(target).mult(settings.seek); 

    this.applyForce(sep); this.applyForce(ali); this.applyForce(coh); this.applyForce(seekForce);
    
    let ns = BOID_CONFIG.noiseStrength;
    this.applyForce(createVector(map(noise(this.zOffset, frameCount * 0.01), 0, 1, -ns, ns), map(noise(this.zOffset + 1000, frameCount * 0.01), 0, 1, -ns, ns)));
  }
  ...
  seek(target) { 
    ...
    steer.limit(BOID_CONFIG.maxForce); 
    return steer; 
  }
  ...
  separate(boids) { 
    ...
    if (c > 0) s.div(c); 
    if (s.mag() > 0) { 
      s.normalize(); s.mult(this.currentMaxSpeed); s.sub(this.vel); s.limit(BOID_CONFIG.maxForce); 
    } 
    return s; 
  }
  
  align(boids) { let s = createVector(); let c = 0; for (let o of boids) { let d = dist(this.pos.x, this.pos.y, o.pos.x, o.pos.y); if (d > 0 && d < BOID_CONFIG.alignDist) { s.add(o.vel); c++; } } if (c > 0) { s.div(c); s.normalize(); s.mult(this.currentMaxSpeed); let st = p5.Vector.sub(s, this.vel); st.limit(BOID_CONFIG.maxForce); return st; } return createVector(); }
  cohesion(boids) { let s = createVector(); let c = 0; for (let o of boids) { let d = dist(this.pos.x, this.pos.y, o.pos.x, o.pos.y); if (d > 0 && d < BOID_CONFIG.cohesionDist) { s.add(o.pos); c++; } } if (c > 0) { s.div(c); return this.seek(s); } return createVector(); }
  edges() { let b = this.baseR * 4; if (this.pos.x > width + b) this.pos.x = -b; if (this.pos.x < -b) this.pos.x = width + b; if (this.pos.y > height + b) this.pos.y = -b; if (this.pos.y < -b) this.pos.y = height + b; }
}